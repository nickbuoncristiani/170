...
def tsp(data):
    ...
    data_list = list(data.nodes())
    data = nx.to_dict_of_dicts(data)

    for vertex in data:
        G[data_list.index(vertex)]={}
        for neighbor in data[vertex]:
            G[data_list.index(vertex)][data_list.index(neighbor)]=data[vertex][neighbor]['weight']
    ...
    #print([data[e] for e in path])
    #print(data)
    return [data_list[e] for e in path]
